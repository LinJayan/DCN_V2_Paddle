...
class DCN_V2Layer(nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
        ...
        self.DeepCrossLayer_ = DeepCrossLayer(sparse_num_field, sparse_feature_dim, dense_feature_dim, 
                                            cross_num, use_low_rank_mixture, low_rank, num_experts)

        self.DNN_ = DNNLayer(sparse_feature_dim, dense_feature_dim, sparse_num_field, layer_sizes, dropout_rate=0.5)
            ...
            self.fc = paddle.nn.Linear(
                ...
                weight_attr=paddle.ParamAttr(
                    initializer=paddle.nn.initializer.Normal(
                        std=1.0 /
                        math.sqrt(self.layer_sizes[-1]))))

        else:
            self.fc = paddle.nn.Linear(
                in_features=self.layer_sizes[-1] + (dense_feature_dim+sparse_num_field)*self.sparse_feature_dim,
                out_features=1,
                weight_attr=paddle.ParamAttr(
                    initializer=paddle.nn.initializer.Normal(
                        std=1.0 /
                        math.sqrt(self.layer_sizes[-1] + dense_feature_dim * sparse_num_field))))
...
class DNNLayer(paddle.nn.Layer):
    def __init__(self, sparse_feature_dim,
                 dense_feature_dim, sparse_num_field, layer_sizes, dropout_rate=0.5):
        ...
        self.input_size = int((self.sparse_num_field+self.dense_feature_dim) * self.sparse_feature_dim)
...
class DeepCrossLayer(nn.Layer):
    def __init__(self, sparse_num_field, sparse_feature_dim, dense_feature_dim, cross_num, 
        ...
        if self.use_low_rank_mixture:
            self.crossNet = CrossNetMix(self.input_dim, layer_num=self.cross_num, low_rank=self.low_rank, num_experts=self.num_experts)
        else:
...
class CrossNetV2(nn.Layer):
    def __init__(self, input_dim, num_layers):
        ...
        self.cross_layers = nn.LayerList(nn.Linear(input_dim, input_dim)
...
class CrossNetMix(nn.Layer):
    ...
    def __init__(self, in_features, layer_num=2, low_rank=32, num_experts=4):
        ...
        self.U_list = paddle.nn.ParameterList([paddle.create_parameter(
            shape=[
                num_experts, in_features, low_rank
            ],
            dtype='float32',
        ...
        self.V_list = paddle.nn.ParameterList([paddle.create_parameter(
            shape=[
                num_experts, in_features, low_rank
            ],
            dtype='float32',
        ...
        # C: (low_rank, low_rank)
        self.C_list = paddle.nn.ParameterList([paddle.create_parameter(
            shape=[
                num_experts, low_rank, low_rank
            ],
            dtype='float32',
            default_initializer=paddle.nn.initializer.XavierNormal()
        ) for i in range(self.layer_num)])

        self.gating = nn.LayerList([nn.Linear(in_features, 1) for i in range(self.num_experts)])

        self.bias = paddle.nn.ParameterList([paddle.create_parameter(
            shape=[
                in_features, 1
            ],
            dtype='float32',
    ...
    def forward(self, inputs):
        ...
        for i in range(self.layer_num):
            ...
            for expert_id in range(self.num_experts):
                ...
                v_x = paddle.matmul(self.V_list[i][expert_id].t(), x_l)  # (bs, low_rank, 1)
                ...
                uv_x = paddle.matmul(self.U_list[i][expert_id], v_x)  # (bs, in_features, 1)
            ...
            output_of_experts = paddle.stack(output_of_experts, axis=2)  # (bs, in_features, num_experts)
            gating_score_of_experts = paddle.stack(gating_score_of_experts, axis=1)  # (bs, num_experts, 1)
            moe_out = paddle.matmul(output_of_experts, F.softmax(gating_score_of_experts, axis=1))
